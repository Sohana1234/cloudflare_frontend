...
async function uploadFile() {
  ...
  if (file.type.startsWith("image/") || file.type === "video/mp4") {
    try {
      ...
      const thumbnailUploadUrl = `https://clodflareapi.pages.dev/api/upload/items/_$flaredrive$/thumbnails/${digestHex}.png`;
      try {
        let thumburl=await axios.post(thumbnailUploadUrl, thumbnailBlob);
        console.log("thumburl",thumburl);
        thumbnailDigest = digestHex;
      } catch (error) {
		console.log("Image error")
        fetch("https://clodflareapi.pages.dev/api/upload/items")
          .then((value) => {
  ...
  try {
    ...
    const uploadUrl = `https://clodflareapi.pages.dev/api/upload/items/${cwd}${uniqueDir}/${file.name}`;
    const headers = {};
