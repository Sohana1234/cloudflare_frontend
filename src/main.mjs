...
export async function multipartUpload(key, file, options) {
  console.log("I am calling multiupload",key)
  const headers = options?.headers || {};
  headers["content-type"] = file.type;

  const uploadId = await axios
    .post(`https://clodflareapi.pages.dev/api/upload/items/${key}?uploads`, "", { headers })
  ...
  const promiseGenerator = function* () {
    for (let i = 1; i <= totalChunks; i++) {
      ...
      yield axios
        .put(`https://clodflareapi.pages.dev/api/upload/items/${key}?${searchParams}`, chunk, {
  ...
  await axios.post(`https://clodflareapi.pages.dev/api/upload/items/${key}?${completeParams}`, {
    parts: uploadedParts,
  });
}
